...
def get_samples(size):
    return [norm.rvs(size=size), cauchy.rvs(size=size), poisson.rvs(10, size=size),
            uniform.rvs(size=size, loc=-m.sqrt(3), scale=2 * m.sqrt(3))]


def get_densities(x):
    return [norm.pdf(x), cauchy.pdf(x), poisson(10).pmf(x), uniform.pdf(x, loc=-m.sqrt(3), scale=2 * m.sqrt(3))]


def get_cdf(x):
    return [norm.cdf(x), cauchy.cdf(x), poisson.cdf(x, mu=10), uniform.cdf(x, loc=-m.sqrt(3), scale=2 * m.sqrt(3))]
